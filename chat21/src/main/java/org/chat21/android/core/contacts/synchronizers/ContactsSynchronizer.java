...
//        });
        ...
        if (contactsChildEventListener == null) {
            ...
            contactsChildEventListener = contactsNode.orderByChild("firstname")
...
//    }
    ...
    public void addContact(IChatUser contact) {
        //filter the contact to remove the current user
        //make a current user number not to be appeared on the contact list
        FirebaseUser currentUser = FirebaseAuth.getInstance().getCurrentUser();
        if (!((currentUser != null) && contact.getId().equals(currentUser.getUid()))) {
            try {
